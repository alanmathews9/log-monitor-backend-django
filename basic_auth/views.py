...
def login(request):
    if 'email_id' not in request.data:
        logger.error(
            'Got login request without email field. Request: {}.'.format(request.data))
        return HttpResponse(json.dumps({"type": "LoginResponse", "status": "failure", "reason": "email id not present"}, default=str),
                            content_type="application/json")
    if 'password' not in request.data:
        logger.error(
            'Got login request without password field. Request: {}.'.format(request.data))
    ...
    status, msg_or_session_id = userstore.login(email_id, password) # get the status and session_id or error message from login method
    # msg_or_session_id will be session_id if status is True else error message
    if status:
...
@csrf_exempt
def logout(request):
    if 'session_id' not in request.data:    # user not logged in
        logger.error(
            'Got logout request without session_id field. Request: {}.'.format(request.data))
    ...
    if not userstore.logout(session_id):    # if logout was not successful
        logger.error(
            'Got logout request with invalid session_id field. Request: {}.'.format(request.data))
...
def register_user(request):
    if 'name' not in request.data:
        logger.error(
            'Got login request without name field. Request: {}.'.format(request.data))
    ...
    status, err_msg = userstore.register(name, email_id, password)  # get the status and error message from register method
    if status:
